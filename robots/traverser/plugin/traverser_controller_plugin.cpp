...
namespace gazebo
{
    ...
    class TraverserControllerPlugin : public ModelPlugin
    {

        public:
            TraverserControllerPlugin();
            ~TraverserControllerPlugin();
...
namespace gazebo
{
    ...
    TraverserControllerPlugin::TraverserControllerPlugin() {}
    ...
    // Load the controller
    void TraverserControllerPlugin::Load(physics::ModelPtr parent,
            sdf::ElementPtr sdf)
    {
        ...
        {
            ROS_INFO_NAMED("traverser_controller_plugin", "TraverserControllerPlugin missing <robotNamespace>, "
        ...
        {
            ROS_WARN_NAMED("traverser_controller_plugin", "TraverserControllerPlugin (ns = %s) missing <commandTopic>, "
                    "defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("traverser_controller_plugin", "TraverserControllerPlugin (ns = %s) missing <odometryTopic>, "
                    "defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("traverser_controller_plugin", "TraverserControllerPlugin (ns = %s) missing <odometryFrame>, "
                    "defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("traverser_controller_plugin", "TraverserControllerPlugin (ns = %s) missing <robotBaseFrame>, "
                    "defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("traverser_controller_plugin", "TraverserControllerPlugin (ns = %s) missing <odometryRate>, "
                    "defaults to %f",
        ...
        {
            ROS_WARN_NAMED("traverser_controller_plugin", "TraverserControllerPlugin (ns = %s) missing <cmdVelTimeout>, "
                    "defaults to %f",
        ...
        {
            ROS_FATAL_STREAM_NAMED("traverser_controller_plugin", "TraverserControllerPlugin (ns = " << robot_namespace_
        ...
        vel_sub_ = nh_->subscribe<geometry_msgs::Twist>(command_topic_, 1,
                &TraverserControllerPlugin::cmdVelCallback, this);
        odometry_pub_ = nh_->advertise<nav_msgs::Odometry>(odometry_topic_, 1);

        // listen to the update event (broadcast every simulation iteration)
        update_connection_ =
            event::Events::ConnectWorldUpdateBegin(
                    boost::bind(&TraverserControllerPlugin::UpdateChild, this));

        ROS_INFO("Traverser controller plugin loaded");
    }

    // Update the controller
    void TraverserControllerPlugin::UpdateChild()
    {
    ...
    void TraverserControllerPlugin::FiniChild()
    {
        nh_->shutdown();
    }

    void TraverserControllerPlugin::cmdVelCallback(const geometry_msgs::Twist::ConstPtr& cmd_msg)
    {
        boost::mutex::scoped_lock scoped_lock(lock);
    ...
    void TraverserControllerPlugin::publishOdometry(double step_time)
    {
    ...
    GZ_REGISTER_MODEL_PLUGIN(TraverserControllerPlugin)
}
